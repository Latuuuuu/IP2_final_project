...
Tool::Tool(const Point &p, const double z, ToolType type) {
	...
	if (type == ToolType::CONCAVE || type == ToolType::CONVEX || type == ToolType::MIRROR) {
		...
		shape.reset(line);
	} else if (type == ToolType::E_FIELD) {
		Square *square = new Square(p.x, p.y, len, z);
		shape.reset(square);
		e_force.first = square->edge_down->x1 - square->edge_up->x1;
		e_force.second = square->edge_down->y1 - square->edge_up->y1;
	}
	this->type = type;
	this->angle = z;
}
