...
public:
	...
	void init() override {
		...
		int tl_y = button_img_top_padding_main;
		int max_height = 0;
		// arrange tower shop
		for(size_t i = 0; i < (size_t)(PauseSetting::ButtonType::BUTTON_MAX); ++i) {
			ALLEGRO_BITMAP *bitmap = IC->get(PauseSetting::button_img_path[i]);
			int w = al_get_bitmap_width(bitmap);
			int h = al_get_bitmap_height(bitmap);
			if(tl_x + w + 100 > DC->window_width) {
				tl_x = DC->game_field_length + button_img_left_padding;
				tl_y += max_height + button_img_h_padding_main;
				max_height = 0;
			}
